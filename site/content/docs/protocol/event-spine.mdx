...
The per-ID cap limits **live supply**. On a burnable edition, burning copies lowers `totalSupply(id)`
and makes that many slots available to mint again; total copies ever issued may therefore exceed the
cap. An ERC-721 Series cap instead limits lifetime-issued IDs, so burning never reopens a slot.
