...
Never lock a token or collection URI to a domain you do not control. A base URI can be changed until
you lock it; after that, it is permanent. If you use a managed resolver, route a domain you control
to it and verify that route before locking. ABX Services does not currently support custom-domain
routing, and the CLI refuses to lock any `abx.io` URI.
...
- **`ok` / `contentIntegrity`** — content **integrity**, and only that: does a served byte carrying
  ...
  a CI job gates on the exit code (or `ok`) and nothing else. Chain-resident `inline`/`reader`
  images have no separate hash copy to compare, so they keep `contentIntegrity: "no-commitments"`;
  the additive `onChainContent` list identifies those sources explicitly.
- **`availability`** — a **sibling** verdict, never a rename of `ok`: is the project's content fully
  *servable* right now. `status` is one of `available` / `partial` / `unavailable` / `unknown`. A
  ...
  Chain-resident image bytes report `available` because they require no external custody.
...
For code projects, `onChainUri.chainComplete` reports whether the template dependency graph resolves
entirely to on-chain bytes. Static image projects have no code dependency graph, so this field is
`null` (not applicable), not `false`.
