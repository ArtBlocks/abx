...
import {EditionCode} from "../tokens/EditionCode.sol";

/// @title EditionCodeFactory — canonical clone factory & trust anchor for code-project editions
/// @notice Deploys {EditionCode} as immutable EIP-1167 clones of a single, factory-owned
///         implementation. The factory is *ownerless* — no admin keys, cannot upgrade or alter
///         any clone — so there is nothing to rug. The exact sibling of {SeriesCodeFactory};
///         each token class is its own trust root (one implementation each), allowlisted by
///         platforms via the factory address.
///
/// @dev Trust model: identical to {SeriesCodeFactory} — trust the factory address once; EIP-1167
...
///
///      **Library linking**: {EditionCode} `delegatecall`s the shared, externally-linked
///      {AbxParamsLib} / {AbxCodeLib} (same non-CREATE2 lib dance as {SeriesCodeFactory}) — `forge
///      script --broadcast` auto-deploys and links them; their addresses land in the broadcast
///      artifacts and belong in the deployment manifest alongside this factory (see
///      `script/DeployEditionCode.s.sol`).
contract EditionCodeFactory {
    ...
    constructor() {
        ...
        implementation = address(new EditionCode());
    }

    /// @notice Deploy a new code-project edition NFT.
    function deploy(EditionCode.InitParams calldata params) external returns (address clone) {
    ...
    function deployDeterministic(EditionCode.InitParams calldata params, bytes32 salt)
        external
    ...
    function _initAndRegister(address clone, EditionCode.InitParams calldata params) internal {
        ...
        EditionCode(clone).initialize(params);
