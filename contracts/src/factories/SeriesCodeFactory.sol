...
import {SeriesCode} from "../tokens/SeriesCode.sol";

/// @title SeriesCodeFactory — canonical clone factory & trust anchor for code projects
/// @notice Deploys {SeriesCode} as immutable EIP-1167 clones of a single, factory-owned
...
///
/// @dev Trust model: identical to {SeriesImageFactory} — trust the factory address once;
///      EIP-1167 guarantees every clone runs the fixed canonical implementation; `isAbxClone`
///      (this factory the only writer) is the membership check. The `AbxDeployed` beacon is
///      spoofable — discovery, never trust. A new core version ships as a new implementation +
///      new factory; old clones stay frozen on their version.
contract SeriesCodeFactory {
    ...
    constructor() {
        ...
        implementation = address(new SeriesCode());
    }

    /// @notice Deploy a new code-project NFT.
    function deploy(SeriesCode.InitParams calldata params) external returns (address clone) {
    ...
    function deployDeterministic(SeriesCode.InitParams calldata params, bytes32 salt)
        external
    ...
    function _initAndRegister(address clone, SeriesCode.InitParams calldata params) internal {
        ...
        SeriesCode(clone).initialize(params);
