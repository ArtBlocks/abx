...
/// @notice Deploys the canonical {OneOfOneImageFactory} (which deploys the
///         implementation). The factory address is the trust root a platform
///         allowlists. Creators then call `factory.deploy(...)`.
contract Deploy is Script {
    ...
    function run() external returns (OneOfOneImageFactory factory) {
        vm.startBroadcast();
        factory = OneOfOneImageFactory(AbxLink.deploy(AbxSalts.ONE_OF_ONE_FACTORY, _initCode(), "OneOfOneImageFactory"));
        vm.stopBroadcast();

        console2.log("OneOfOneImageFactory:", address(factory));
    ...
    function predict() external view {
        console2.log("OneOfOneImageFactory", AbxLink.predict(AbxSalts.ONE_OF_ONE_FACTORY, _initCode()));
    }
    ...
    function _initCode() private view returns (bytes memory) {
        return AbxLink.initCode("OneOfOneImageFactory", AbxLink.imageLibs());
    }
}
